...
if [ "$RELEASE_STATUS" == "DEV" ]; then
    PACKAGE_NAME="$NAME_LOWER-$VERSION-$COMMIT_SHORT-$RELEASE_STATUS-$OS-$ARCH"
    WINDOWS_INSTALLER_VERSION_TAG="$VERSION ($COMMIT_SHORT) $RELEASE $RELEASE_STATUS"
else
    PACKAGE_NAME="$NAME_LOWER-$VERSION-$OS-$ARCH"
    WINDOWS_INSTALLER_VERSION_TAG="$VERSION $RELEASE"
fi
